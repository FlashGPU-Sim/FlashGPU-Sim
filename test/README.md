...
cd test
./run_tests.sh setup
./run_tests.sh run test --target sm100 --group unit
...
```bash
# SM100/B200 FA4 and TCGen05 correctness
./run_tests.sh run test --target sm100 --group unit
...
- **SM100_B200**: default SM100/B200 configuration for FA4 and TCGen05 tests.
...
Use `SM100_B200` for SM100/FA4 tests, `SM120_RTX5090` for SM120 tests, and
`SM90_H100` for Hopper tests unless an experiment explicitly requires another
...
│   ├── sm100
│   │   └── unit
│   │       ├── FA4 opaque tensor-map decoding
│   │       └── TCGen05 TMEM/descriptor/MMA model
│   ├── sm120
│   │   ├── unit
│   │   │   ├── basic
│   │   │   ├── bulk-group
