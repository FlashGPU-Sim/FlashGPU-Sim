...
def load_manifest(path: Path) -> ArchitectureManifest:
    ...
    for index, source in enumerate(raw_sources):
        ...
        if (
            ...
            raise ManifestError(
                f"{label} must be a normalized path below test/src/<test_group>/"
            )
        ...
        if not (SOURCE_DIR / posix_path).is_file():
            raise ManifestError(f"{label} does not exist below test/src: {source}")
...
def render_make(manifests: tuple[ArchitectureManifest, ...]) -> str:
    lines = [
        "# Generated from test/arch/*.toml by scripts/arch_manifest.py.",
