...
VARIANTS=(baseline nothing only_cp_async only_softmax only_mma softmax_mma)
SEQLENS=(256 512 1024 2048 4096)
  ...
  for variant in "${VARIANTS[@]}"; do
    ...
    for mode in full causal; do
      for seqlen in "${SEQLENS[@]}"; do
        if [[ "${mode}" == "full" ]]; then
          case_name="H1D128FullB1S${seqlen}"
        else
...
print_cases() {
  ...
  while IFS= read -r row; do
    IFS='|' read -r case_id variant case_name batch seqlen heads head_dim mode gtest_filter binary <<<"${row}"
    echo "${case_id},${variant},${case_name},${batch},${seqlen},${heads},${head_dim},${mode},${gtest_filter},${binary}"
  done < <(emit_cases)
}
