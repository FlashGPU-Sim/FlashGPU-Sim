...
mapfile -t MANIFEST_ARCHITECTURES < <(
  make -s --no-print-directory -C test list-architectures
)
...
architecture_config() {
  ...
  metadata="$(make -s --no-print-directory -C test \
...
test_group_supported() {
  ...
  done < <(make -s --no-print-directory -C test list-test-groups ARCH="$arch")
  return 1
}
...
run_core_test_group() {
  ...
  if [ "$arch" = sm90 ] && [ "$test_group" = integration ]; then
    run_logged sm90-cp-async-src-size-stats \
      python3 test/validators/cp_async_src_size_stats.py \
