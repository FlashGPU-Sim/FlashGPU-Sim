...
usage() {
  cat <<'EOF'
usage: test/run_fa4_b200_suite_matrix.sh [options]

Run a full FA4 workload suite by grouping test/fa4_b200_cases.csv rows by the
generated artifact specialization: direction, head_dim, head_dim_v, dtype, and
causal mode.  Each group is delegated to run_fa4_b200_cases.sh.
...
artifact_variants() {
  "$ROOT_DIR/test/run_fa4_b200_cases.sh" --suite "$SUITE" --list |
    awk -F, '
      NF {
        key = $6 "," $7 "," $8 "," $9
...
main() {
  ...
  for variant in "${variants[@]}"; do
    ...
    local cmd=(
      "$ROOT_DIR/test/run_fa4_b200_cases.sh"
      --config "$CONFIG"
      --suite "$SUITE"
      --direction "$DIRECTION"
      --run-dir "$sub_run_dir"
