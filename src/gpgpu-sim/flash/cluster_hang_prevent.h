...
// Env always wins. Without env, the watchdog is off unless cluster DSM /
// remote mbarrier is enabled, so mixed bar.sync + try_wait on ordinary
// (non-cluster) kernels cannot abort flash configs.
inline unsigned hang_watchdog_threshold(unsigned configured,
                                        bool cluster_features = true) {
