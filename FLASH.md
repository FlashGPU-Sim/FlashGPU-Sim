...
2. **Commit/wait groups**: `cp.async.bulk.commit_group` and `cp.async.bulk.wait_group` treated as NOPs (src/gpgpu-sim/flash/tma.cc:1121-1127)
   - **Note**: These stubbed instructions are explicitly allowed during PTX inspection for TMA tests
3. **Tensormap options**: Some tensormap manipulation options not validated (src/gpgpu-sim/flash/tma.cc:1273)
4. **Multi-dimensional testing**: Full test coverage for 1D and 3D-5D tensor operations is implemented in `test/src/integration/cuda_tma_multidim_test.cc`
