...
- Remote-load completion is not a baked `2×hop` issue stall.
- Fabric `read_data` must complete through LDST writeback, not as a no-op.
...
Local SMEM / self-mapa latency stays on the SMEM path (`-gpgpu_smem_latency`
and related knobs), not a hop table.
...
Do not write peer smem at issuer execute on the fabric path.
...
Per-bank SRAM is optional follow-up, not required for this shipping model.
...
## 7. Fabric remote-load path (shipped)

1. `decode_space` marks `m_dsm_remote` and resolves the owner CTA.
2. `ld_impl` does **not** fill dest RF; it notes a fabric `read_command`.
3. `func_exec_inst` reclassifies dest regs as shared. The warp parks on the
   local SMEM scoreboard / LDST path.
4. After `read_data` + SRAM grant, writeback fills RF and releases the
   scoreboard.

Remote DSM without `-gpgpu_dsm_enable 1` aborts. Do not fill dest RF at
functional execute on the fabric path.
