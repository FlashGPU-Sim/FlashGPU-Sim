...
Remote: `remote_mbarrier_*` in `mbarrier.cc`. Packets: mbarrier request/completion on the intra-GPC fabric (`-gpgpu_dsm_enable 1`).
...
|------|---------|------------------|
| TB-cluster home | One GPC | One `simt_core_cluster` (GPC) |
...
| DSM load completion | Hop, remote SRAM, hop, RF | Fabric `read_data` + SRAM + local SMEM writeback |
| DSM bandwidth | ~21 B/cycle shaped | Shaper 2/3 × 32 B payload on CLUSTER132 |
