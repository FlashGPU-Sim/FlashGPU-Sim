...
`dsm_fabric_t` moves bytes between SMs in one GPC:
...
A 128 B read response occupies **four** 32 B **payload** grants. A real package may also carry a header; this model does not charge extra occupancy for that header.
...
Required packet fields: `packet_id`, `transaction_id`, `network_src_sm_id`, `network_dst_sm_id`, `transaction_requester_sm_id`, `transaction_target_sm_id`, `vc`, `packet_class`, `payload_bytes` (v2 `wire_bytes` = payload in v1), `total_flits`, `remaining_flits`, `route_lane`, `created_cycle`, `injected_cycle`, `tail_arrival_cycle`.
...
A 4-of-6 TDM is a simulation of that rate, as is `hard_rate_cap`. That TDM is **not** a confirmed hardware timetable. Also support a 2-of-3 skip with **different SMs skipping different cycles**.
...
`skip_mod` and `fixed_tdm` have the same **average** rate and **different co-eligible sets**. That can matter for the blog’s adjacent-pair result. Keep both and do not hard-code 21.0 as a silicon constant.
...
No requester/service port, shader-side, or memory-side in this API. `can_inject` false → **LSU stall**. Do not map a full buffer to an immediate peer write.
...
## 7.1 Stats and transaction dump
...
- Map a full buffer to an immediate peer write

Extract generic **bounded VOQ, RR arbiter, credit, stats** if useful. Do **not** instantiate two `xbar_router`s as DSM VCs. Do not inherit shader-side vs memory-side ports.
...
| | `dsm_fabric_t` |
|--|----------------|
| Timing | Flit grants + credits + shaper |
| Load data | After `read_data` tail + SRAM service |
| Store | `write_data` flits + coalesced ACK |
| BW | 32 B payload × shaped slots |
| Topology | GPCMMU hash + GX + GPCARB |
