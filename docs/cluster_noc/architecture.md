...
| DSM endpoint | `dsm_endpoint_id_t` | Unique inside one GPC |
...
Keep `-gpgpu_n_clusters` / `-gpgpu_n_cores_per_cluster` as aliases of
`-gpgpu_num_gpcs` / `-gpgpu_num_sms_per_gpc`. If old and new knobs disagree
at start-up, **abort**.
...
### Global interconnect stays per SM
...
Putting several SMs in one GPC must **not** collapse shader icnt nodes to one
per GPC. Each enabled SM has its own global interconnect endpoint.
...
`simt_core_cluster` owns `dsm_fabric_t` / `dsm_endpoint_protocol_t` when
`-gpgpu_dsm_enable 1`. OpenMP is already per physical cluster. Each enabled
SM has its own global icnt endpoint.
