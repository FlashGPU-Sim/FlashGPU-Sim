...
The blog uses the same arithmetic. **Do not** promote an implementation choice (TDM vs skip_mod) to a silicon fact.
...
| TMA multicast network is unresolved | Existing measurements do not establish that multicast shares the DSM fabric | Functional fan-out plus fixed completion latency only; no multicast network/contention model |
...
| Policy | Notes |
|--------|-------|
| `fixed_tdm` | CPC **slots** `{0,1,2,3}` / `{2,3,4,5}` / `{0,1,4,5}` — four live clients per phase, matches four lanes |
| `skip_mod` | Skip 1 of 3 cycles, phase = `sm_id % 3` or `cpc_slot % 3`. Same 2/3 **rate**, **different** co-eligible sets than TDM |
| `hard_rate_cap` | Equivalent average, no idle reuse |

`skip_mod` by global `sm_id` and TDM by CPC slot are **not** the same concurrent set. Adjacent-pair grouping in the blog is **measured** under contention. Do not hard-code one policy as hardware.

---

## 4. Unresolved (must stay knobs)

From the blog’s “not bit accurate” list:
...
| Topic | This spec |
|-------|-----------|
| TPC / TPCARB | **Not modeled**; SM → GPCMMU |
| 32 B unit | 32 B **payload** per grant; extra header width unmodeled (128 B = 4 grants) |
| Shaper default | Both TDM and `skip_mod`; `skip_mod` phase offset required |
| GX count | Knob, default 2 |
| PG'd SM slots | In the topology table (`num_sms_per_gpc` may be less than 6×CPCs) |
| Remote load completion | Same **SMEM** scoreboard / LDST writeback as local `ld.shared` |
| Cluster barriers | Timing state is held by the physical cluster and keyed by TB-cluster group |
