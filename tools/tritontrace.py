...
This module tracks Triton kernel compilation and invocation and extracts
standalone binaries for simulation with FlashGPU-Sim.
...
class Tracker:
    ...
    def __init__(self, output_dir, save_binaries: bool = True, capture_args: bool = True, enabled: bool = True):
        ...
        print(f"[TritonTracker] Initialized")
        ...
        def patched_run(jit_self, *args, **kwargs):
            ...
            if tracker_self.enabled and tracker_self.capture_args and not warmup:
                ...
                if grid is not None:
                    ...
                    tracker_self.pending_grid = Tracker._normalize_grid(grid)
    ...
    def _on_launch_exit(self, launch_metadata):
        ...
        if self.capture_args and self.pending_args is not None and self.pending_args_snapshots is not None and HAS_TORCH:
            print(f"\n[TritonTracker] Detecting and capturing outputs for launch #{launch_info.launch_id}")
    ...
    def _on_launch_enter(self, launch_metadata):
        ...
        print(f"\n[TritonTracker] Launch #{self.launch_counter}: {kernel_name}")
