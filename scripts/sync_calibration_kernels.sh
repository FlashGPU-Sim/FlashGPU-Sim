#!/usr/bin/env bash
# Populate the git-ignored calibration/ tree from sibling checkouts and the
# pinned dsm_bw commit. Safe to re-run; does not touch git-tracked files.
set -euo pipefail
...
if [[ -d "${DEST}/kernels/h200_probes" ]]; then
  patch -d "${DEST}/kernels/h200_probes" -p1 \
    < "${ROOT}/scripts/calibration_overlays/h200_probes_sim.patch"
fi
...
This directory is **not** part of the FlashGPU-Sim git tree. It holds synchronized
kernel snapshots and local simulator runs. `h200_probes` currently mirrors the
updated vendor-first `../H200_profiling` suite. Hardware results are pending.
The supervisor-facing report is:
...
Keep only `calibration/results/*final*`; other result directories are disposable.
Do not commit anything under `calibration/`.
EOF
